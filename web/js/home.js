...
document.addEventListener('DOMContentLoaded', async () => {
    initNav();
    initParticles();
    await updateBlogHeader();
    await Promise.all([loadRecentPosts(), loadStats()]);
});

/* ── Blog Header + Nav ── */
async function updateBlogHeader() {
    const blog = await getBlogInfo();
    const subtitle = document.querySelector('.hero-sub');
    if (subtitle && blog.label !== 'Machine Learning Blog') {
        subtitle.textContent = blog.label + ' — Explore our latest articles.';
    }

    const mlCats = ['ml', 'dl', 'nlp', 'cv', 'genai', 'statistics', 'ainews'];
    const cats = blog.profile === 'ml-blog' ? mlCats : Object.keys(CATEGORY_META);

    const navLinks = document.getElementById('navLinks');
    if (navLinks) {
        let navHTML = '<li><a href="index.html" class="nav-link active">Home</a></li>';
        for (const cat of cats) {
            const meta = CATEGORY_META[cat];
            if (!meta) continue;
            navHTML += `<li><a href="category.html#cat=${cat}" class="nav-link" data-cat="${cat}">${meta.shortLabel}</a></li>`;
        }
    ...
    if (footerCats) {
        ...
        for (const cat of cats) {
            const meta = CATEGORY_META[cat];
            if (!meta) continue;
            footerHTML += `<a href="category.html#cat=${cat}">${meta.shortLabel}</a>`;
        }
        footerCats.innerHTML = footerHTML;
    }

    const statLabel = document.querySelector('#totalCategories')?.querySelector('.stat-label');
    if (statLabel) statLabel.textContent = 'Topics';
}
